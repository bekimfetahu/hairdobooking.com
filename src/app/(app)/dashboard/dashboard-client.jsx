...
export default function DashboardClient({ initialUser }) {
    const user = useSelector((state) => state.auth.user) || initialUser;
        ...
        >
            <section className="grid gap-4 md:grid-cols-3">
                {quickStats.map((stat) => {
                    ...
                    return (
                        <article key={stat.label} className="rounded-md border border-black/10 bg-white p-5 shadow-sm">
            ...
            <section className="mt-6 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                <article className="rounded-md border border-black/10 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-6 grid gap-3 sm:grid-cols-2">
                        {shortcuts.map((item) => {
                            ...
                            return (
                                <div key={item.title} className="rounded-md border border-black/10 bg-neutral-50 p-4">
                ...
                <aside className="rounded-md border border-black/10 bg-black p-6 text-white shadow-sm">
                    ...
                    <div className="mt-6 space-y-3">
                        {['Book a salon visit', 'See upcoming appointments', 'Manage favorites'].map((item) => (
                            <div key={item} className="rounded-md border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/90">
                                {item}
                            </div>
                        ))}
                    </div>
